...
import { BackBar, SectionLabel, TechChip, linkPillClass, useSkipEntrance } from "@/components/view-parts";
          ...
          <NewInfo delay={0.15} className="mt-2 flex flex-col gap-2">
            ...
            <div className="flex flex-col overflow-hidden rounded-[14px] border border-[#e2e8f0] bg-[#f8fafc] dark:border-[rgba(255,255,255,.09)] dark:bg-[rgba(255,255,255,.03)]">
              {products.map((prod, i) => (
                <motion.div
                  key={prod.name}
                  initial={skipEntrance ? false : { opacity: 0, x: -16 }}
                ...
                >
                  {/* No 01/02/03 counter: the order of a company's products
                      carries nothing the reader needs, and the dividers already
                      separate them. */}
                  <div className="flex min-w-0 flex-1 flex-col gap-1.5">
                    <div className="flex flex-wrap items-baseline justify-between gap-2">
                      <h3 className="font-sora text-[14px] font-bold text-[#0f172a] dark:text-[#f1f5fb]">
                        {prod.name}
                      </h3>
                      {prod.link && prod.url && (
